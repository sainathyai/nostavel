...
tools: Read, Grep, Glob, Edit, Write, Bash, PowerShell, mcp__playwright
model: sonnet
skills:
  - implement-frontend
  - verify-change
hooks:
  PreToolUse:
    - matcher: "Bash|PowerShell|Write|Edit|MultiEdit|NotebookEdit"
      hooks:
        - type: command
          command: "node \"${CLAUDE_PROJECT_DIR}/scripts/agent-guards/hook.mjs\" --role frontend-engineer"
          timeout: 15
---
...
You may edit only: `src/app/**/*.tsx`, `src/components/**`, `src/app/globals.css`, `public/**`. Anything else belongs to another role; hand it off through the ticket or pull request. The repository guard enforces this.
